...
## 1. 소유자 (Owner)
- **정의:** 최종 결정자(항상 필수).
- **입력:** Architect/Analyst가 작성한 검토 대상(시스템 정의, QS, Proposed ADR, 후보/평가, `architecture-brief.md`/views).
...
- **출력:** 후보 구조(동작/개발 view), 채택 ADR 제안(Proposed), `architecture-brief.md` + `arch/views/`(arc42/C4).
...
- **입력:** 후보 구조, 우선순위 QS, `architecture-brief.md`/views.
